class Theory::KeySignature
  ...
  def num_sharps
    (Theory::Circle.of_fifths.pitch_classes.index(tonic_pitch_class) - scale_type_adjustment) % 12
  end

  def num_flats
    (Theory::Circle.of_fourths.pitch_classes.index(tonic_pitch_class) + scale_type_adjustment) % 12
  end
